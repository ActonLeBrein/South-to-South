...
uniApp.controller('indexCtrl', function($scope, $filter, uniFactory) {
	...
	$scope.check = function(){
		...
		var arr1 = $.map($scope.model_origin_esp, function(n, i) { if (n) return i; });
		var arr2 = $.map($scope.model_destiny_esp, function(n, i) { if (n) return i; });
		//console.log("check");
	...
	$scope.init = function(){
		...
		$.each(json, function(index,value) {
			...
			if ($.inArray(DynNam, $scope.model_origin_esp) == -1) {
				$scope.model_origin_esp[DynNam.split(' ')[0].split('á')[0].split('é')[0].split('í')[0].split('í')[0].split('ú')[0].split('ñ')[0]] = false;
			}
			if ($.inArray(DynNam, $scope.model_destiny_esp) == -1) {
				$scope.model_destiny_esp[DynNam.split(' ')[0].split('á')[0].split('é')[0].split('í')[0].split('í')[0].split('ú')[0].split('ñ')[0]] = false;
			}
			...
			if (!$scope.paises[value.iso_dest]){				
				$scope.paises[value.iso_dest] = {};
				$scope.paises[value.iso_dest].iso = value.iso_dest;
				$scope.paises[value.iso_dest].esp = value.paises;
				$scope.paises[value.iso_dest].ing = value.countries;
			}
		});				
				...
				.on("click", function(d) {
					if ($(this).attr('id')){
						...
						centers = {'mexic':[-100,-200],'guate':[-385,-327],'hondu':[-385,-327],'salva':[-385,-327],'nicar':[-385,-327],
								   ...
								   'colom':[-744,-660],'venez':[-944,-660],'guyana':[-944,-660],'suriname':[-944,-660],'guiana':[-944,-660],'ecuad':[-644,-760],'per':[-844,-1060],'boliv':[-944,-1260],
	...
	$scope.normalizar = function(name){
		...
		switch (name){
			...
			default	:
				r = name.split(' ')[0].split('á')[0].split('é')[0].split('í')[0].split('í')[0].split('ú')[0].split('ñ')[0];
				break;
		}
		//console.log("Normalizar: "+r);
		return r;		
	}
	...
	$scope.viewYear = function(y){
		...
		$.each(json, function (index,value) {
			...
			if ((value.iso_orig != "USA") && (value.iso_dest != "USA")){
				var name = $scope.normalizar(value.ctry_orig_esp);
				var dest = $scope.normalizar(value.ctry_dest_esp);
	...
	$scope.agregarComas = function(numero){
